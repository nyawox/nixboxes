  ...
  config = mkIf cfg.rdp {
    ...
    systemd = {
      user.services.waydroid-rdp = {
        ...
        serviceConfig = {
          ExecStart = ''
            ${pkgs.weston}/bin/weston --config=/etc/waydroid-cloud.ini --shell=kiosk --backend=rdp --rdp-tls-cert=/var/lib/waydroid/waydroid-rdp.crt --rdp-tls-key=/var/lib/waydroid/waydroid-rdp.key --height=1846 --width=980 --port=3150 --no-clients-resize
          '';
          Restart = "on-failure";
          RestartSec = "5s";
        };
